...
.args <- if (interactive()) sprintf(c(
  ...
  file.path('output', 'sim_plot_third.png')
...
plot_sim <- function(dat, sim) (ggplot(dat) 
                                ...
                                + geom_line(aes(y = ma_third), color = '4')
                                + geom_point(aes(y = third), size = .2, color = '4', alpha = .5)
                                + geom_line(data = sim, aes(y = exp_reinf), color = 'darkgrey')
                                + ylab('Third Infection')
...
inc_reinf_fit <- (plot_sim(ts[date < as.Date(fit_through)], eri[date < as.Date(fit_through)])
                  + ggtitle('Fitting period')
                  + geom_text(aes(label = year, y = 0), data = ts[date < as.Date(fit_through), .(year = format(date, '%Y'), date)][, .(date = min(date)), by = year], vjust = -43, hjust = 'left', nudge_x = 14, size = 7*0.35)
)

inc_reinf_proj <- (plot_sim(ts[date > as.Date(fit_through)], eri[date > as.Date(fit_through)])
                   + ggtitle('Projection period')
                   + geom_text(aes(label = year, y = 0), data = ts[date > '2022-01-31', .(year = format(date, '%Y'), date)][, .(date = min(date)), by = year], vjust = -43, hjust = 'left', nudge_x = 14, size = 7*0.35)
)
